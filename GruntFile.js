...
module.exports = function(grunt) {
	...
	grunt
			.initConfig({
				...
				includeSource : {
					options : {
						basePath : '<%= config.path.webapp.root %>',
					},
					myTarget : {
						files : {
							'<%= config.path.webapp.root %>/index.html' : '<%= config.path.webapp.root %>/index.tpl.html'
						}
					}
				},

				// From grunt-contrib-concat. This is usefull when we have more
				// than one css file, not the case now...
				/*
				 * concat: { styles: { src: [ '<%= config.path.webapp.root
				 * %>/css/style.css', ], dest: '<%= config.path.temp.root
				 * %>/concat/css/application.css' } },
				 */

				// From grunt-contrib-copy. Copies remaining files to places
				// other tasks can use
				copy : {
					build : {
						files : [ {
				...
				// From grunt-usemin. Reads HTML for usemin blocks to enable
				// smart builds
				useminPrepare : {
	...
	// 'concat:styles',
	'includeSource', 'wiredep', 'useminPrepare', 'concat:generated', 'cssmin',
			'uglify', 'copy:build', 'usemin', 'htmlmin' ]);
